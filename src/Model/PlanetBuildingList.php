...
class PlanetBuildingList implements Iterator, ArrayAccess
{
    ...
    public static function fromPlanet(Planet $planet): PlanetBuildingList
    {
        $buildings = [];
        foreach ($planet->getPlanetBuildings() as $building) {
            $buildings[$building->getName()] = $building->getLevel();
        }

        return new PlanetBuildingList($buildings);
    }
}